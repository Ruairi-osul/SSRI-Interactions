...
from ssri_interactions.plots import heatmap, wrap_all_text, wrap_ylabel, PAL_GREY_BLACK
...
from ssri_interactions.spiketrains.waveforms import WaveformPreprocessor
...
class NeuronTypesFigurePreprocessor:
    def __init__(
        ...
        metrics: Sequence[str] = ("Waveform Width (ms)", "CV(ISI)", "Spike Rate (Hz)",),
    ...
    def preprocess_single_unit(
        self, single_unit_props: pd.DataFrame, single_unit_neuron_types: pd.DataFrame,
    ) -> pd.DataFrame:
        df_single_unit = (
    ...
    def preprocess_all(
        ...
        df_probe = self.preprocess_probe(
            probe_spiketrain_props, probe_waveform_props, probe_neuron_types,
        )
        df_single_unit = self.preprocess_single_unit(
            single_unit_props, single_unit_neuron_types,
        )
...
class NeuronPropsPlotter:
    ...
    def neuron_type_heat(
        ...
        heatmap(
            df_binned_piv=df,
            heatmap_kwargs=dict(
                vmin=vmin, vmax=vmax, cmap=cmap, cbar_ax=cbar_ax, cbar=plot_cbar,
            ),
            ax=ax,
            locater_x=locater_x,
...
class NeuronTypesPlotter:
    ...
    def plot_heatmaps(
        ...
        axes = fig.subplot_mosaic(
            [["h1", "h2", "c",], ["h1", "h2", ".",],], gridspec_kw=grid_spec_kwargs,
        )
    ...
    def plot_bars(
        ...
        for col_idx, electrode_type in enumerate(electrode_types):
            for row_idx, metric in enumerate(metrics):
                ...
                sns.barplot(
                    data=df_, x=neuron_type_col, y=value_col, ax=ax, **barplot_kwargs,
                )
                if col_idx == 0:
